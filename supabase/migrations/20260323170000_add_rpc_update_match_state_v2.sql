-- Non-overloaded match-state RPC to avoid PostgREST overload/schema-cache issues.
-- Keep this self-contained so it does not depend on older function variants existing.

CREATE OR REPLACE FUNCTION public.rpc_update_match_state_v2(
  p_match_id uuid,
  p_status text,
...
GRANT EXECUTE ON FUNCTION public.rpc_update_match_state_v2(uuid, text, int) TO authenticated;
