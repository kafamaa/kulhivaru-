-- Tournament wizard persistence (categories + format rules)

CREATE TABLE IF NOT EXISTS public.tournament_categories (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tournament_id uuid NOT NULL REFERENCES public.tournaments(id) ON DELETE CASCADE,
  -- The wizard draft provides stable category ids; we store them to map format_rules back.
  ...
  roster_min int NOT NULL,
  roster_max int NOT NULL,
  ...
  UNIQUE (tournament_id, source_category_id)
);

CREATE TABLE IF NOT EXISTS public.tournament_format_rules (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tournament_id uuid NOT NULL REFERENCES public.tournaments(id) ON DELETE CASCADE,
  ...
  teams_advance_per_group int NOT NULL,
  include_best_runners_up int NOT NULL,

  knockout_round text,
  round_robin_legs int NOT NULL,
  third_place_match boolean NOT NULL DEFAULT false,

  tiebreak_order jsonb,

  auto_generate_fixtures boolean NOT NULL DEFAULT true,
  ...
  max_matches_per_day_per_team int NOT NULL,
  min_rest_minutes_between_matches int NOT NULL,

  created_at timestamptz DEFAULT now()
);

ALTER TABLE public.tournament_categories ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.tournament_format_rules ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow authenticated read tournament_categories" ON public.tournament_categories;
CREATE POLICY "Allow authenticated read tournament_categories"
  ON public.tournament_categories FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow authenticated write tournament_categories" ON public.tournament_categories;
CREATE POLICY "Allow authenticated write tournament_categories"
  ON public.tournament_categories FOR ALL TO authenticated USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow authenticated read tournament_format_rules" ON public.tournament_format_rules;
CREATE POLICY "Allow authenticated read tournament_format_rules"
  ON public.tournament_format_rules FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow authenticated write tournament_format_rules" ON public.tournament_format_rules;
CREATE POLICY "Allow authenticated write tournament_format_rules"
  ON public.tournament_format_rules FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Extend publish RPC to persist categories + format rules.
...
BEGIN
  ...
  -- Persist wizard categories + format rules (if present in payload).
