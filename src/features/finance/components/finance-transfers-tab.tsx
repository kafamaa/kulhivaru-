...
import type { FinanceAccount, FinanceTransferRow } from "../types";
...
export function FinanceTransfersTab(props: {
  tournamentId: string;
  transfers: FinanceTransferRow[];
  paymentAccounts: FinanceAccount[];
}) {
  const { tournamentId, transfers, paymentAccounts } = props;
  const router = useRouter();
  const [showAddModal, setShowAddModal] = useState(false);
  const [fromAccountId, setFromAccountId] = useState(paymentAccounts[0]?.id ?? "");
  const [toAccountId, setToAccountId] = useState(paymentAccounts[1]?.id ?? paymentAccounts[0]?.id ?? "");
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm text-slate-400">
          Move money between Cash, Bank, and other accounts.
        </p>
        <button
          type="button"
          ...
          onClick={() => {
            ...
            setToAccountId(paymentAccounts[1]?.id ?? paymentAccounts[0]?.id ?? "");
            setAmount("");
          }}
          disabled={isPending || paymentAccounts.length < 2}
        >
          Add transfer
        </button>
      </div>
      ...
      {paymentAccounts.length < 2 ? (
        <div className="rounded-xl border border-slate-800 bg-slate-950 p-4">
          <p className="text-sm text-amber-200">
            You need at least 2 payment accounts (Cash, Bank) to record transfers.
          </p>
        </div>
      ) : transfers.length === 0 ? (
        <div className="rounded-xl border border-slate-800 bg-slate-950 p-6 text-center">
          <p className="text-sm text-slate-300">No transfers yet.</p>
          <p className="mt-1 text-xs text-slate-500">
            Record movements between Cash and Bank accounts.
          </p>
          <button
            type="button"
          ...
          <div className="divide-y divide-slate-800">
            {transfers.map((t) => (
              <div key={t.id} className="grid grid-cols-12 gap-2 px-4 py-3 text-sm">
                <div className="col-span-3 truncate text-slate-200">{t.fromAccountName}</div>
                <div className="col-span-3 truncate text-slate-200">{t.toAccountName}</div>
                <div className="col-span-2 font-medium tabular-nums text-slate-50">{moneyToString(t.amount)}</div>
                <div className="col-span-2 truncate text-slate-400">{t.reference ?? "—"}</div>
          ...
          <div className="w-full max-w-xl rounded-2xl border border-slate-800 bg-slate-950 p-4 sm:p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="space-y-1">
                <span className="block text-xs text-slate-400">From account</span>
                <select
              ...
              <label className="space-y-1">
                <span className="block text-xs text-slate-400">To account</span>
                <select
                  className="w-full rounded-xl border border-slate-800 bg-slate-900/40 px-3 py-2 text-sm text-slate-100"
                  value={toAccountId}
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                ...
                onClick={submitAdd}
                disabled={isPending || !amount.trim() || fromAccountId === toAccountId}
              >
